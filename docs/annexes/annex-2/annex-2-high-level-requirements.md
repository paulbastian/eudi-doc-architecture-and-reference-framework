...
| WTE_07 | During the activation of a new Wallet Instance, a Wallet Provider SHALL sign the Wallet Trust Evidence (WTE) and issue it to the Wallet Instance. |
| WTE_08 | The Commission SHALL take measures to ensure that the contents, format and encoding of the Wallet Trust Evidence is specified in a technical specification. Each WTE SHALL describe the certified WSCA available to that Wallet Instance, in such a way that a PID Provider or Attestation Provider processing the WTE is able to take a well-grounded decision on whether to issue an attestation to that Wallet Instance and WSCA. <br>Notes: <ul><li>In general, a PID Provider or Attestation Provider will also need information about the User. Such information is not contained in the WTE.</li><li>The WTE contains information allowing a PID Provider or Attestation Provider to verify whether the WTE is compromised and therefore revoked. Revocation information for the WTE is defined in Topic 7.</li></ul> |
...
| **Index** | **Requirement specification** |
|------------|-------------------|
| WIRevocation_01 | To enable a Relying Party or an Attestation Provider to verify the authenticity and revocation status of a Wallet Instance it is interacting with, a Wallet Provider SHALL issue one or more WIAs to the Wallet Instance during or after the activation phase of a Wallet Instance. |
...
| **Index** | **Requirement specification** |
|------------|------------------|
| WIRevocation_06 | A Wallet Provider SHALL be able to suspend a Wallet Instance, by suspending its WIA(s) as specified in \[[Topic 7](#a237-topic-7---attestation-validity-checks-and-revocation)\] or by stopping issuance of short-lived WIAs. |
| WIRevocation_07 | A Wallet Provider SHALL be able to revoke a Wallet Instance, by revoking its WIA(s) as specified in \[[Topic 7](#a237-topic-7---attestation-validity-checks-and-revocation)\] or by stopping issuance of short-lived WIAs. |
| WIRevocation_08 | A Wallet Provider SHALL be able to unsuspend a Wallet Instance, by unsuspending its WIA(s)  or by resuming issuance of short-lived WIAs. |
